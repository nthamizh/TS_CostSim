export function SegmentCell({
  v, s, old, isFirst, placeholder,
}: {
  v: string|null; s?: string; old?: string|null;
  isFirst?: boolean; placeholder?: boolean;
}) {
  ...
  const cls = placeholder
    ? "text-gray-400 italic"           // static placeholder — muted italic
    : s === "ff"    ? "bg-amber-50 text-amber-700 font-semibold"
    : s === "pers"  ? "bg-blue-50 text-blue-700"
    : s === "dept"  ? "bg-purple-50 text-purple-700"
    : s === "elig"  ? "bg-green-50 text-green-700 font-semibold"
    : s === "cost"  ? "bg-gray-100 text-gray-500"
    : s === "ppg"   ? "bg-teal-50 text-teal-700 font-semibold"
    : s === "segov" ? "bg-pink-50 text-pink-700 font-semibold"
    : "text-gray-700";
